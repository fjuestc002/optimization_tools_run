"""R-NSGA-III: Reference point-based many-objective evolutionary algorithm."""
...
class RNSGA3(NSGA3):
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize R-NSGA-III algorithm with aspiration point-based survival.

        Args:
            ref_points: Reference points for the algorithm.
            pop_per_ref_point: Size of the population used for each reference point.
            mu: Shrinkage factor (0-1) for aspiration points. Smaller = tighter
                convergence, larger = wider spread.
            ...
            n_offsprings: Number of offspring to generate per generation.
            **kwargs: Additional arguments passed to parent NSGA3 algorithm.
        """
...
def get_ref_dirs_from_points(ref_point, ref_dirs, mu=0.1):
    """Generate reference directions around user-specified reference points.

    Creates smaller sets of equidistant Das-Dennis points around the projection
    of user points on the Das-Dennis hyperplane.

    Args:
        ref_point: User-specified reference points.
        ref_dirs: Reference directions (Das-Dennis points).
        mu: Shrinkage factor (0-1). Smaller = tighter convergence,
            larger = wider spread.

    Returns:
        Array of reference directions adjusted around user reference points.
    """
...
def line_plane_intersection(l0, l1, p0, p_no, epsilon=1e-6):
    """Compute intersection of a line with a plane.

    Reference: https://en.wikipedia.org/wiki/Line%E2%80%93plane_intersection

    Args:
        l0: Starting point of the line.
        l1: Ending point of the line.
        p0: A point on the plane.
        p_no: Normal vector of the plane (does not need to be normalized).
        epsilon: Tolerance for parallel check.

    Returns:
        Intersection point as a vector, or perpendicular projection if the
