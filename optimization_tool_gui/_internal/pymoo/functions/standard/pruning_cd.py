...
def calc_pcd(X, n_remove=0):
    """Calculate pruning with crowding distance.

    Args:
        X: Objective values array of shape (N, M).
        n_remove: Number of points to remove.
    ...
    I = np.argsort(X, axis=0, kind="mergesort")  # noqa: E741
    ...
    while n_removed < (n_remove - 1):
        ...
        I = np.argsort(X[H].copy(), axis=0, kind="mergesort")  # noqa: E741
