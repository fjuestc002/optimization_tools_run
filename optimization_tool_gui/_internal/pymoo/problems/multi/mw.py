"""Multi-objective MW problems.

Based on the C++ implementation by Ma and Wang.
...
class MW(Problem):
    def __init__(self, n_var: int, n_obj: int, n_ieq_constr: int, **kwargs) -> None:
        if "xl" not in kwargs:
            kwargs["xl"] = 0
        if "xu" not in kwargs:
            kwargs["xu"] = 1
    ...
    @staticmethod
    def LA2(A: float, B: float, C: float, D: float, theta: np.ndarray) -> np.ndarray:
        return A * np.power(np.sin(B * np.power(theta, C)), D)

    @staticmethod
    def LA3(A: float, B: float, C: float, D: float, theta: np.ndarray) -> np.ndarray:
...
class MW1(MW):
    def __init__(self, n_var: int = 15, **kwargs) -> None:
    ...
    def _calc_pareto_front(self, ref_dirs: np.ndarray | None = None) -> np.ndarray:  # noqa: ARG002
        if ref_dirs is None:
            F = np.zeros((100, 2))
            ...
            F = ref_dirs
        F[:, 1] = 1 - 0.85 * F[:, 0]
        length = np.sqrt(2) * F[:, 1] - np.sqrt(2) * F[:, 0]
        c = 1 - F[:, 0] - F[:, 1] + 0.5 * np.sin(2 * np.pi * length) ** 8
        F = F[c >= 0]
        return F
...
class MW4(MW):
    def __init__(self, n_var: int | None = None, n_obj: int = 3, **kwargs) -> None:  # noqa: PYI041
        if n_var is None:
            n_var = n_obj + 12
    ...
    def _calc_pareto_front(self, ref_dirs: np.ndarray | None = None) -> np.ndarray:  # noqa: ARG002
        ...
        F = ref_dirs
        length = F[:, -1] - np.sum(F[:, :-1], axis=1)
        c = (1 + 0.4 * np.sin(2.5 * np.pi * length) ** 8) - np.sum(F, axis=1)
        return F[c >= 0]
...
class MW5(MW):
    ...
    def _calc_pareto_front(self, **kwargs) -> np.ndarray:  # type: ignore[return-value]  # noqa: ARG002
        return Remote.get_instance().load("pymoo", "pf", "MW", "MW5.pf")  # type: ignore[return-value]
...
class MW6(MW):
    ...
    def _evaluate(self, X: np.ndarray, out: dict, *args, **kwargs) -> None:  # type: ignore[override]  # noqa: ARG002
        ...
        g0 = (
            f0**2 / np.power(1.0 + self.LA3(0.15, 6.0, 4.0, 10.0, atan), 2.0)
            + f1**2 / np.power(1.0 + self.LA3(0.75, 6.0, 4.0, 10.0, atan), 2.0)
            - 1
        )
        out["F"] = np.column_stack([f0, f1])
        out["G"] = g0.reshape((-1, 1))
...
class MW7(MW):
    ...
    def _calc_pareto_front(self, **kwargs) -> np.ndarray:  # type: ignore[return-value]  # noqa: ARG002
        return Remote.get_instance().load("pymoo", "pf", "MW", "MW7.pf")  # type: ignore[return-value]
...
class MW9(MW):
    ...
    def _calc_pareto_front(self, **kwargs) -> np.ndarray:  # type: ignore[return-value]  # noqa: ARG002
        return Remote.get_instance().load("pymoo", "pf", "MW", "MW9.pf")  # type: ignore[return-value]
...
class MW10(MW):
    ...
    def _calc_pareto_front(self, **kwargs) -> np.ndarray:  # type: ignore[return-value]  # noqa: ARG002
        return Remote.get_instance().load("pymoo", "pf", "MW", "MW10.pf")  # type: ignore[return-value]
...
class MW11(MW):
    ...
    def _calc_pareto_front(self, **kwargs) -> np.ndarray:  # type: ignore[return-value]  # noqa: ARG002
        return Remote.get_instance().load("pymoo", "pf", "MW", "MW11.pf")  # type: ignore[return-value]
...
class MW12(MW):
    ...
    def _evaluate(self, X: np.ndarray, out: dict, *args, **kwargs) -> None:  # type: ignore[override]  # noqa: ARG002
        ...
        g0 = (
            -1.0
            * (1 - 0.625 * f0 - f1 + 0.08 * np.sin(2 * np.pi * (f1 - f0 / 1.6)))
...
class MW13(MW):
    ...
    def _evaluate(self, X: np.ndarray, out: dict, *args, **kwargs) -> None:  # type: ignore[override]  # noqa: ARG002
        ...
        g0 = (
            -1.0
            * (5.0 - (1 + f0 + 0.5 * f0 * f0) - 0.5 * np.sin(3 * np.pi * f0) - f1)
    ...
    def _calc_pareto_front(self, **kwargs) -> np.ndarray:  # type: ignore[return-value]  # noqa: ARG002
        return Remote.get_instance().load("pymoo", "pf", "MW", "MW13.pf")  # type: ignore[return-value]
...
class MW14(MW):
    ...
    def _calc_pareto_front(self, **kwargs) -> np.ndarray:  # type: ignore[return-value]  # noqa: ARG002
        if self.n_obj == 3:
            return Remote.get_instance().load("pymoo", "pf", "MW", "MW14.pf")  # type: ignore[return-value]
        return np.array([])
