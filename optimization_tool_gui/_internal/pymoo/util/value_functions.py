...
def create_vf_scipy_poly(
    ...
    def hess_func(x):  # noqa: ARG001
        return np.zeros((len(x0), len(x0)))

    if method != "trust-constr":
        hess_func = None  # type: ignore[assignment]
    ...
    # package up results
    def vf(P_in):  # noqa: ARG001
        return poly_vf(P_in, res.x[0:-1])
...
def create_vf_scipy_linear(
    ...
    # package up results
    def vf(P_in):  # noqa: ARG001
        return linear_vf(P_in, res.x[0:-1])
...
def create_vf_pymoo_linear(P, ranks, delta, eps_max, method="ES", verbose=False):
    ...
    res = moomin(vf_prob, algorithm, ("n_gen", 200), verbose=verbose, seed=1)
...
def create_vf_pymoo_poly(
    ...
    res = moomin(vf_prob, algorithm, ("n_gen", 100), verbose=verbose, seed=1)
...
def _build_ineq_constr_poly(P, vf, ranks, delta):
    ...
    def ineq_func(x):  # noqa: ARG001
        return _ineq_constr_poly(x, P, vf, ranks, delta)
...
def _build_ineq_constr_linear(P, vf, ranks, delta):
    ...
    def ineq_func(x):  # noqa: ARG001
        return _ineq_constr_linear(x, P, vf, ranks, delta)
...
def _build_constr_linear(P, vf, ranks, delta):
    ...
    def constr_func(x):  # noqa: ARG001
        return np.append(ineq_constr_func(x), _eq_constr_linear(x))
...
def _calc_S(P, x):  # noqa: ARG001
    ...
    l_matrix = x[M * M : (M * M) + M].reshape(M, 1)
    l_matrix = l_matrix.T

    # Calc S for an arbitrary dimensional space of P
    S = np.matmul(P, k) + (l_matrix * 2)
